...
struct ExtensionFeaturePair
{
  ...
  uint32_t    specVersion      = 0;        // [optional] Spec version of the extension
...
private:
  ...
  void createDevice()
  {
    ...
    VkResult res = vkCreateDevice(m_physicalDevice, &createInfo, m_settings.alloc, &m_device);
    if(res != VK_SUCCESS)
                                 ...
                                 std::vector<ExtensionFeaturePair>&        filteredExtensions)
  {
    ...
    for(const auto& desiredExtension : desiredExtensions)
    {
      ...
      if(it != availableExtensionsMap.end()
         && ((desiredExtension.exactSpecVersion == false && desiredExtension.specVersion <= it->second)
             || (desiredExtension.exactSpecVersion && desiredExtension.specVersion == it->second)))
      {
      ...
      else
      {
        ...
        {
          LOGE("Extension not available: %s (version %s%d)\n", desiredExtension.extensionName, desiredExtension.exactSpecVersion?"==":">=", desiredExtension.specVersion);
          allFound = false;
        }
        else
          LOGW("Extension not available: %s (version %s%d)\n", desiredExtension.extensionName, desiredExtension.exactSpecVersion ? "==" : ">=", desiredExtension.specVersion);
      }
    }
